...
async function redirect2Pan(r) {
    //根据实际情况修改下面的设置
    const embyHost = 'https://emby-8920-stardust2714.cloud.okteto.net'; //这里默认emby/jellyfin的地址是宿主机,要注意iptables给容器放行端口
    const embyMountPath = '/mnt';  // rclone 的挂载目录, 例如将od, gd挂载到/mnt目录下:  /mnt/onedrive  /mnt/gd ,那么这里 就填写 /mnt
    const alistToken = 'alist-46c063fc-742b-4c22-8caf-a5a2c16d0859G5UWLPtW5OjXae7EadEJk9kuKBdFCGLoFVZAX6FLAse2S2yXwXIeerdBwLgnQo5E';      //alist token, 在alist后台查看
    const alistAddr= 'https://mtfl-alist.run.goorm.app'; //访问宿主机上5244端口的alist地址, 要注意iptables给容器放行端口
    const embyApiKey = '8124f1054dc8463d9e61d6df2479c102';  //emby/jellyfin api key, 在emby/jellyfin后台设置
    const alistPublicAddr = 'https://mtfl-alist.run.goorm.app'; // alist公网地址, 用于需要alist server代理流量的情况, 按需填写
